...
#define DEVICE_NAME                         "BinaryKeyboard"                          
// Manufacturer. Will be passed to Device Information Service.
#define MANUFACTURER_NAME                   "Ianstuff"                         
...
// The advertising duration of fast advertising in units of 10 milliseconds. 
#define APP_ADV_FAST_DURATION               3000                                       
// The advertising duration of slow advertising in units of 10 milliseconds. 
#define APP_ADV_SLOW_DURATION               18000                                      


//lint -emacro(524, MIN_CONN_INTERVAL)
// Loss of precision 
...
// Maximum length of Feature Report. 
...
// Position of the modifier byte in the Input Report. 
#define MODIFIER_KEY_POS                    0                                          
// The start position of the key scan code in a HID Report. 
...
// Variable to indicate if Caps Lock is turned on.
...
static void bas_evt_handler(ble_bas_t *p_bas, ble_bas_evt_t *p_evt)
{
	switch (p_evt->evt_type)
	{
	    case BLE_BAS_EVT_NOTIFICATION_ENABLED:
			NRF_LOG_INFO("BAS NOTIFICATONS ENABLED");
			bas_active = true;
			break;
	    case BLE_BAS_EVT_NOTIFICATION_DISABLED:
			NRF_LOG_INFO("BAS NOTIFICATONS DISABLED");
			bas_active = false;
	}
}
...
void battery_level_update(uint8_t level)
{
	...
	if (bas_active)
	{
		NRF_LOG_INFO("SENDING BATTERY LEVEL: %d", level);
...
static void bas_init(void)
{
    ...
    bas_init_obj.evt_handler          = bas_evt_handler;
